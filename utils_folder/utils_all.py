...
def generate_bin_matrix_from_freqs(thetas, N, seed = 0):
    ...
    Output:
        X <array> shape N, K; binary, with X[n,k] = 1(n-th individual displays k-th feature), X[n,k] ~ Bern(thetas[k])
    '''
    np.random.seed(seed)
...
def generate_cts_from_bin_mat(X):
    '''
    Input:
        X <array> shape N,K, binary valued
    Output:
        cts <array> shape N+1; n-th entry is # distinct variants observed in first n-1 individual; first entry is 0
    '''
...
def count_new_freq(binary_matrix, N, M, r):
    ...
    Input:
        ...
        r <int> frequency
    Output:
        cts <int> number of variants not observed in first N samples and appearing with frequency r in M new samples
    '''    
